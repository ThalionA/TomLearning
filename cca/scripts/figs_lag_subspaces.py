...
def fig_stability(df: pd.DataFrame, fs: str):
    ...
    for ax, pair in zip(axes, PAIRS):
        ...
        for col, colour, label in [("angle_x", C_X, ax_name),
                                   ("angle_y", C_Y, ay_name)]:
            ...
            ax.plot(lags, mean, "-o", color=colour, lw=1.6, ms=3, zorder=3,
                    label=f"{label} (n={piv.shape[0]})")
            ax.fill_between(lags, mean - sem, mean + sem, color=colour, alpha=0.18,
                            zorder=2)
        fl = np.nanmean([sub["floor_x"].mean(), sub["floor_y"].mean()])
        fl_sd = np.nanstd(pd.concat([sub["floor_x"], sub["floor_y"]]).to_numpy(float),
                          ddof=1)
    ...
    fig.suptitle(f"Subspace stability across lag — {fs}, 10 ms smoothed | grey band = "
                 "split-half noise floor (inside = same subspace)", fontsize=12)
...
def fig_fffb(df: pd.DataFrame, fs: str):
    ...
    for ax, pair in zip(axes, PAIRS):
        ...
        ang = np.nanmean([ff.loc[common, "angle_ff_fb_x"].mean(),
                          ff.loc[common, "angle_ff_fb_y"].mean()])
        flr = np.nanmean([ff.loc[common, "floor_x"].mean(),
                          ff.loc[common, "floor_y"].mean()])
...
def main():
    ...
    df = pd.read_csv(src)
    for c in ("cc1", "angle_x", "angle_y", "floor_x", "floor_y", "angle_ff_fb_x",
              "angle_ff_fb_y", "gini_x_conn", "gini_y_conn"):
